...
describe('Debounce', () => {
  ...
  it('should debounce class instances separately', () => {
    const foo1 = new Foo()
    const foo2 = new Foo()
    foo1.debouncedByDefault('foo1-a')
    foo2.debouncedByDefault('foo2-a')
    expect(foo1._debouncedByDefault).to.equal(undefined)
    expect(foo1._debouncedByDefaultCount).to.equal(0)
    expect(foo2._debouncedByDefault).to.equal(undefined)
    expect(foo2._debouncedByDefaultCount).to.equal(0)
    timers.runTimersToTime(DEFAULT_DEBOUNCE_DURATION-1)
    foo1.debouncedByDefault('foo1-b')
    timers.runTimersToTime(1)
    expect(foo1._debouncedByDefault).to.equal(undefined)
    expect(foo1._debouncedByDefaultCount).to.equal(0)
    expect(foo2._debouncedByDefault).to.equal('foo2-a')
    expect(foo2._debouncedByDefaultCount).to.equal(1)
    timers.runTimersToTime(DEFAULT_DEBOUNCE_DURATION-1)
    expect(foo1._debouncedByDefault).to.equal('foo1-b')
    expect(foo1._debouncedByDefaultCount).to.equal(1)
    expect(foo2._debouncedByDefault).to.equal('foo2-a')
    expect(foo2._debouncedByDefaultCount).to.equal(1)
  })
})
